...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
    ...
    if(op==A_ADD){
        ...
        ovf=((argA>0)!=(argB>0))&&((argB>0)!=(val>0));
    }
...
stat_t nexti(y64sim_t *sim)
{
    ...
    if (icode != I_ALU && icode != I_JMP && icode != I_RRMOVQ && ifun != 0) {
    ...
    switch (icode) {
        ...
        case I_MRMOVQ:
        case I_ALU:
            if (!get_byte_val(sim->m, next_pc, &regist)) {
                err_print("PC = 0x%x, Invalid instruction address", sim->pc);
                return STAT_ADR;
            }
            regA = GET_REGA(regist);
            regB = GET_REGB(regist);
            if (!NORM_REG(regA) || !NORM_REG(regB)) {
                err_print("PC = 0x%x, Invalid register id %.2x", sim->pc, regist);
                return STAT_INS;
            }
            next_pc++;
            break;
        ...
        case I_POPQ:
            ...
            regA = GET_REGA(regist);
            regB = GET_REGB(regist);
            if (!NONE_REG(regB) || !NORM_REG(regA)) {
                err_print("PC = 0x%x, Invalid register id %.2x", sim->pc, regist);
                return STAT_INS;
            }
            next_pc++;
    ...
    long_t valueA,valueB,valueMem,rsp,ret_add,result,pop;
    ...
    switch (icode) {
        ...
        case I_RRMOVQ:  /* 2:x regA:regB */
            ...
            if (cond_doit(sim->cc, ifun)) {
                valueA = get_reg_val(sim->r, regA);
                set_reg_val(sim->r, regB, valueA);
            }
            sim->pc = next_pc;
            break;
        ...
        case I_RMMOVQ: /* 4:0 regA:regB imm */
            valueA = get_reg_val(sim->r, regA);
            valueB = get_reg_val(sim->r, regB);
            if (!set_long_val(sim->m, valueB + imm, valueA)) {
                err_print("PC = 0x%x, Invalid data address %p", sim->pc, (void*)(valueB + imm));
                return STAT_ADR;
            }
            sim->pc = next_pc;
            break;
        case I_MRMOVQ: /* 5:0 regB:regA imm */
            valueB = get_reg_val(sim->r, regB);
            valueMem;
            if (!get_long_val(sim->m, valueB + imm, &valueMem)) {
                err_print("PC = 0x%x, Invalid data address %p", sim->pc, (void*)(valueB + imm));
                return STAT_ADR;
            }
            set_reg_val(sim->r, regA, valueMem);
            sim->pc = next_pc;
            break;
        case I_ALU: /* 6:x regA:regB */
            ...
            valueA = get_reg_val(sim->r, regA);
            valueB = get_reg_val(sim->r, regB);
            result = compute_alu(ifun, valueA, valueB);
        ...
        case I_CALL: /* 8:x imm */
            ...
            if (!set_long_val(sim->m, rsp, next_pc)) {
                err_print("PC = 0x%x, Invalid stack address %p", sim->pc, (void*)rsp);
                return STAT_ADR;
            }
            sim->pc = imm; 
            break;
        case I_RET: /* 9:0 */
            ...
            if (!get_long_val(sim->m, rsp, &ret_add)) {
                err_print("PC = 0x%x, Invalid stack address %p", sim->pc, (void*)rsp);
                return STAT_ADR;
            }
        ...
        case I_PUSHQ: /* A:0 regA:F */
            valueA = get_reg_val(sim->r, regA);
            valueB = get_reg_val(sim->r, REG_RSP)-8;
            set_reg_val(sim->r, REG_RSP, valueB);
            if (!set_long_val(sim->m, valueB, valueA)) {
                err_print("PC = 0x%x, Invalid stack address %p", sim->pc, (void*)valueB);
                return STAT_ADR;
            }
            sim->pc = next_pc;
            break;
        case I_POPQ: /* B:0 regA :F */
            valueB = get_reg_val(sim->r, REG_RSP);
            if (!get_long_val(sim->m, valueB, &pop)) {
                err_print("PC = 0x%x, Invalid stack address %p", sim->pc, (void*)valueB);
                return STAT_ADR;
            }
            set_reg_val(sim->r, REG_RSP, valueB+8);
            set_reg_val(sim->r, regA, pop);
            sim->pc = next_pc;
            break;
        default:
            err_print("PC = 0x%lx, Invalid instruction %.2x", sim->pc, codefun);
