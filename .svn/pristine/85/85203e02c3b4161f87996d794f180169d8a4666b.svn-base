...
{   
    ...
    if(!builtin_cmd(argv)){
        Sigprocmask(SIG_BLOCK, &mask_one, &prev); //Block SIGCHLD to avoid deleting before adding
        if((pid=fork())==0){
            Sigprocmask(SIG_SETMASK,&prev,NULL);//UnBlock SIGCHLD
            if(setpgid(0, 0)<0)	//Puts the child in a new process group whose group ID is identical to the childs PID
            {
        ...
        if(!bg){
            ...
            // Print jid, pid and command for background
            Sigprocmask(SIG_BLOCK, &mask_all, NULL);
...
int builtin_cmd(char **argv) 
{
    ...
    else if (!strcmp(argv[0], "jobs")) {//print jobs
        sigset_t mask_all, mask_one, prev;
        Sigfillset(&mask_all);	
        Sigemptyset(&mask_one);
        Sigaddset(&mask_one, SIGCHLD);
    ...
    else if (!strcmp(argv[0], "bg")||!strcmp(argv[0], "fg")) {//fg or bg
        do_bgfg(argv);
        return 1;
    }
...
void do_bgfg(char **argv) 
{
    if (argv[1] == NULL)//no pid or job id
    {
    ...
    if (sscanf(argv[1], "%%%d", &id) > 0)// job id
    {
...
{
    while(fgpid(jobs)>0)
        sleep(1);//wait until foreground process finish
    return;
}
